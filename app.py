...
# import mysql.connector  # Commented out — analysis disabled for now
import requests
from datetime import datetime, timedelta
import pytz
from geopy.geocoders import Nominatim
from timezonefinder import TimezoneFinder
# import pandas as pd        # Commented out — analysis disabled for now
...
# # MySQL Database Connection  — commented out, analysis disabled for now
...
# # ======================== Analysis Route (disabled for now) ========================
...
#         try:
#             conn = get_db_connection()
#             query = "SELECT date, temperature, humidity FROM weather_data1 WHERE city = %s AND date BETWEEN %s AND %s"
...
#             plt.plot(df['date'], df['temperature'], label='Temperature', color='red', marker='o')
#             plt.plot(df['date'], df['humidity'], label='Humidity', color='blue', linestyle='--', marker='s')
#             plt.legend(); plt.title(f"Weather Analysis: {city}")
#             plt.xlabel("Date"); plt.ylabel("Values"); plt.grid(True); plt.tight_layout()
